...
//                color: styleData.value > gauge.value ? root.gaugeRearColor : (styleData.value > gauge.value-1 ? "red" : "root.gaugeFrontColor")
        ...
        Text {
            id: gaugeValue
            text: root.value.toFixed(decimal)
            color: root.centerColor
            anchors { centerIn: parent }
            font.family: digital.name
            font.pixelSize: Math.min(gauge.width/ 4, gauge.width/Math.max((root.maximumValue+ "").length, (root.minimumValue + "").length)) // Scale the gauge font based on what the largest or smallest number that might be displayed
        ...
        CircularGauge {
            ...
            maximumValue: root.maximumValue.toFixed(2)
